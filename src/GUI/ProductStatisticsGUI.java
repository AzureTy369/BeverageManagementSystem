...
public class ProductStatisticsGUI extends JPanel {
    ...
    private void loadBestSellingProducts() {
        ...
        List<ProductDTO> bestSellingProducts = productController.getBestSellingProducts(limit);
        ...
        for (ProductDTO product : bestSellingProducts) {
            ...
            List<ProductDetailDTO> details = productDetailController.getProductDetailsByProduct(product.getProductId());
            double price = 0;
            if (!details.isEmpty()) {
                // Use the first detail's price or calculate average price if needed
                price = details.get(0).getPrice();
            }
